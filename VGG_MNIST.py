...
num_channels = 1
LearningRate = 1e-4
num_kernels = [1, 1, 2, 2]  # VGG7 architecture
NumClasses = 10
specPoolSize = 1
kernel_size = [3, 3]
stride = [2, 2]
...
def count_batch_norm_params(num_features):
    ...
    # Gamma (scale) + Beta (shift) + moving mean + moving variance
    # For trainable: gamma + beta = 2 * num_features
...
def dct_domain_convolution(dct_input, out_channels, layer_idx):
    ...
    # Weight parameters (real-valued for DCT)
    ...
    # Element-wise multiplication in DCT domain
    out = []
...
# FLOPs calculation
def compute_dct_domain_flops_improved(input_size, num_channels, num_kernels, kernel_size, stride):
...
def compute_dct_memory_access_improved(input_size, num_channels, num_kernels, kernel_size, stride):
...
# Create 4 convolutional layers in DCT domain (VGG7)
...
for N in range(len(num_kernels)):
    with tf.variable_scope('dct_conv' + str(N)):
        ...
        # Count parameters for this conv layer
        ...
        parameter_counts['batch_norm_params'] += bn_trainable  # Only count trainable BN params
...
# Create 3 fully connected layers (VGG7 style)
...
# Calculate total parameters (including BN params)
...
with tf.name_scope('loss'):
    Loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(
        labels=OneHotLabels, logits=FC3))
...
total_flops = compute_dct_domain_flops_improved(input_size, num_channels, num_kernels, kernel_size, stride)
total_memory_accesses = memory_accesses['read_accesses'] + memory_accesses['write_accesses']
total_memory_access = compute_dct_memory_access_improved(input_size, num_channels, num_kernels, kernel_size, stride)
memory_cost = total_memory_access * 4 / (1024 * 1024)

# Print results
print("\n" + "="*75)
print("FINAL RESULTS - DCT-VGG7 on MNIST Dataset")
print("="*75)
