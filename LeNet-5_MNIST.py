...
def count_parameters(var_list):
    """Count total and trainable parameters from variable list"""
    total = 0
    for var in var_list:
...
# Simple FLOP calculation function
...
for N in range(len(num_kernels)):
    with tf.variable_scope('conv' + str(N)):
        ...
        # Note: We'll calculate based on kernel dimensions and channel counts
...
parameter_counts['total_params'] = parameter_counts['conv_total'] + parameter_counts['fc_total']
...
# Print results with parameter information
print("\n" + "="*70)
print("FINAL RESULTS - DCT-LeNet on Custom Dataset (Simple Calculation)")
print("="*70)
print("\n--- MODEL PARAMETERS ---")
